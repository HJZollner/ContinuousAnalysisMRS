...
% Set up auto email:
opts.mailto.config = '/Users/chris_davies-jenkins/Local/Scratch/AutoAnalysisTest/EmailConfig.json'; % Path to config file
opts.mailto.recipients = {'cdavies9@jh.edu','c.w.daviesjenkins@gmail.com'}; % Cell array of email recipients
...
data_folder = fileparts(which(fullfile('exampledata','nifti-mrs','jobNIfTIMRS.m'))); 
...
for kk = 1:length(subs)
    ...
    for ll = 1:length(sess)
        ...
        % default.
        dir_ref    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_SVS-ref.nii.gz']);
